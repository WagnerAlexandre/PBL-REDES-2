...
// Desenvolvido com o meu paradigma favorito: XGH
...
func getUnicaChaveContasFromBG(cpfCnpj string) ([]Conta, error) {
	resp, err := http.Get("http://" + HOST + ":65502/getUnicaChaveContas?cpf_cnpj=" + cpfCnpj)
	if err != nil {
		return nil, err
	}
	defer resp.Body.Close()
...
func procuraConta(c *gin.Context) {
	...
	if banco == ENDPNT {
		...
		if contaPF, ok := TContasPF[numConta]; ok {
			...
			c.SetCookie("alvoBBMNtransfer", cookieValue, 3600, "/", HOST, false, false)
...
func retornarContas(c *gin.Context) {
	...
	cookieParts := strings.Split(fmt.Sprintf("%v", cookie), "|")
	...
	bbContas, err := getUnicaChaveContasFromBB(cpfCnpj)
	if err == nil {
		contas = append(contas, bbContas...)
	}
...
func loginHandler(c *gin.Context) {
	...
	if conta != nil {
		...
		c.SetCookie("BBMN", cookieValue, 3600, "/", HOST, false, false)
